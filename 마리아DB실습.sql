...
DESC tb_int;
/*
AUTO_INCREMENT : 테이블 생성시 자동증가 컬럼으로 지정할때 사용하는
	제약조건. 오라클은 테이블과 별도로 시퀀스를 생성하지만, MySQL의
	경우 테이블의 특정 컬럼에 지정한다. 
UNSIGNED : 숫자 컬럼인 경우 주로 -100~100과 같이 음수와 양수의 범위를
	사용하게 되는데, 양수만 사용하고 싶다면 unsigned를 지정한다. 이 경우
	음수의 범위만큼 양수의 범위가 2배로 늘어나게 된다. 
*/
SELECT * FROM tb_int;
# 자동증가 컬럼으로 지정한 idx는 쿼리에서 제외한다. 
INSERT INTO tb_int (num1, num2, num3, num4, fnum1, fnum2)
	VALUES (101, 12346, 1234568, 1234567891, 
		12345.12345 , 1234567890.12345678791); #2번 실행해서 레코드 삽입

SELECT * FROM tb_int;
/*
자동증가 컬럼에 임의의 값을 삽입할 수는 있으나, 사용하지 않는것을
권장한다. 
*/
...
SELECT * FROM tb_int;
# idx컬럼은 PK로 지정했으므로 빈값은 삽입할 수 없다. 에러발생됨.
INSERT INTO tb_int (idx, num1, num2, num3, num4, fnum1, fnum2)
	VALUES ('', 101, 12346, 1234568, 1234567891, 
		12345.12345 , 1234567890.12345678791); 
...
CREATE TABLE tb_date (
	...
	date2 DATETIME DEFAULT CURRENT_TIMESTAMP #날짜와시간까지 표현
);
DESC tb_date;	
/*
CURRENT_TIMESTAMP : 날짜 타입의 컬럼에 디폴트값으로 현재시간을
	입력한다. 

NOW() : insert쿼리에서 날짜타입의 컬럼에 현재시간을 입력할때 사용하는
	함수.	
*/		
...
MySQL에서는 짧은글인 경우 varchar(크기), 긴 글인 경우 text를 사용한다.
*/
...
#MariaDB에서는 commit 이 필요없다. 삽입한 데이터는 자동커밋 된다. 
...
/*
	MySQL에서는 게시물의 구간을 정해 가져오기 위해 limit 를 사용한다. 
	형식]
		.. 쿼리문 limit 시작위치, 레코드갯수;
*/
...
CREATE TABLE mvcboard (
	...
	ofile VARCHAR(200),
	sfile VARCHAR(30),
	downcount SMALLINT DEFAULT 0 NOT NULL,
