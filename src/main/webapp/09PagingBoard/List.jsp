...
//자바는 나눠도 소수점이 안나오기 때문에 double로 해줘야함
//전체 페이지 수를 계산한다.
int totalPage = (int)Math.ceil((double)totalCount / pageSize);
//int totalPage = ((totalCount / pageSize)+1); //이렇게 하면 안되나? 되네
...
//게시물의 구간을 계산한다.
/*
한페이지에 출력할 게시물의 범위를 결정한다.  MySQL에서는 limit를
사용하므로 start(시작위치)만 계산하면 된다.
	start => 
		1페이지일때 : (1-1) * 5 = 0
		2페이지일때 ; (2-1) * 5 = 5
	end =>
		한페이지 출력할 게시물의 갯수인 pageSize를 그대로 사용하면 된다.  
*/
...
<%
      	...
      	for (BoardDTO dto : boardLists) {
        	//virtualNum = totalCount--; //전체 게시물 수에서 시작해서 1씩 감소한다.
...
%>
    ...
    <table border="1" width="90%">
        <tr align="center">
        	<!-- 페이징 처리 -->
        	<td>
        		<!-- 항상 바뀌는 파일명을 가져오기 위해 reqUrl = request.getRequestURI()  -->
